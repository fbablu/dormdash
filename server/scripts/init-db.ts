...
async function initializeDatabase() {
  ...
  const restaurantsData: Restaurant[] = JSON.parse(
    fs.readFileSync(path.join(__dirname, '../../../ton_restaurants.json'), 'utf8')
  );
  ...
  try {
    ...
    const [cuisinesRows] = await connection.execute<mysql.RowDataPacket[]>('SELECT id, name FROM cuisines');
