...
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    const restaurantDoc = await restaurantsCollection.doc(orderData.restaurantId).get();
    ...
    const deliveryFee = orderData.deliveryFee || restaurantData?.deliveryFee || 3.99;
...
export const updateOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (orderData.status === 'delivered' || orderData.status === 'cancelled') {
      return res.status(400).json({ error: 'Cannot update a completed or cancelled order' });
    }

    // Customer can only cancel their own orders
    if (status === 'cancelled' && req.user.uid !== orderData.customerId && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Forbidden - Only the customer or admin can cancel an order' });
    }
...
export const acceptDelivery = async (req: Request, res: Response) => {
  try {
    ...
    if (orderData.status !== 'pending') {
      return res.status(400).json({ error: 'Order is no longer available for delivery' });
    }
    
    // Check that user is not trying to deliver their own order
    if (orderData.customerId === req.user.uid) {
      return res.status(400).json({ error: 'You cannot deliver your own order' });
    }
