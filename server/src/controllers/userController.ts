...
export const verifyDorm = async (req: Request, res: Response) => {
  try {
    ...
    if (userId !== req.user.uid && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Forbidden - Cannot verify another user's dorm" });
    }
...
export const getFavorites = async (req: Request, res: Response) => {
  try {
    ...
    if (userId !== req.user.uid && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Forbidden - Cannot access another user's favorites" });
    }
...
export const toggleFavorite = async (req: Request, res: Response) => {
  try {
    ...
    if (userId !== req.user.uid && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Forbidden - Cannot modify another user's favorites" });
    }

    if (!restaurantName || !action) {
      return res
        .status(400)
        .json({ error: "Restaurant name and action are required" });
    }
