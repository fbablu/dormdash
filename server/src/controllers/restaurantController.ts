...
export const getRestaurantMenu = async (req: Request, res: Response) => {
  try {
    ...
    // Get menu categories
    const categoriesSnapshot = await restaurantsCollection.doc(id).collection('menu').get();
...
export const addOrUpdateRestaurant = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user.role !== 'admin' && req.user.role !== 'restaurant_owner') {
      return res.status(403).json({ error: 'Forbidden - Admin or restaurant owner access required' });
    }

    const { id } = req.params;
    const restaurantData = req.body;
    
    if (!restaurantData.name || !restaurantData.location || !restaurantData.address) {
    ...
    if (id) {
      ...
      const newRestaurant: Omit<Restaurant, 'id'> = {
        ...
        createdAt: now,
        updatedAt: now,
        ownerId: req.user.role === 'restaurant_owner' ? req.user.uid : undefined
      };
