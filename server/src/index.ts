...
app.post('/api/auth/google', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      // Generate JWT token
      const token = jsonwebtoken.sign({ id: userId, email, name }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/api/orders', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { restaurantId, totalAmount, deliveryFee, deliveryAddress, notes } = req.body as OrderData;
    ...
    try {
      await connection.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [orderId, customerId, restaurantId, totalAmount, deliveryFee, deliveryAddress, notes]
      );
...
// Get user's orders
app.get('/api/user/orders', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
...
app.get('/api/delivery/requests', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const connection = await pool.getConnection();
    try {
      const [orders] = await connection.execute<mysql.RowDataPacket[]>(
...
app.post('/api/delivery/accept/:orderId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      ...
      if (orders[0].status !== 'pending' || orders[0].deliverer_id !== null) {
        await connection.rollback();
        return res.status(400).json({ error: 'Order is already assigned or not pending' });
      }
      ...
      res.status(200).json({ message: 'Delivery request accepted successfully' });
    } catch (error) {
      await connection.rollback();
      throw error;
    } finally {
...
// Update order status
app.put('/api/orders/:orderId/status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { orderId } = req.params;
    ...
    try {
      ...
      if (order.deliverer_id !== userId && (status !== 'cancelled' || order.customer_id !== userId)) {
