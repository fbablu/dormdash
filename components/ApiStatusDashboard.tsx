...
const ApiStatusDashboard = () => {
  ...
  const [authState, setAuthState] = useState<{ isLoggedIn: boolean; userId?: string; email?: string }>({
    isLoggedIn: false
  });
  ...
  const endpoints = [
    ...
    { name: '/api/users/favorites', path: '/api/users/favorites', method: 'GET', needsAuth: true },
    { name: '/api/restaurants', path: '/api/restaurants', method: 'GET' },
    { name: '/api/user/profile', path: '/api/user/profile', method: 'GET', needsAuth: true },
  ];
      ...
      <View style={styles.statusCard}>
        ...
        <View style={styles.statusRow}>
          ...
          <View style={styles.statusIndicatorContainer}>
            ...
            <Text style={{ color: authState.isLoggedIn ? '#4caf50' : '#f44336' }}>
              ...
              <View key={index} style={styles.endpointItem}>
                ...
                {endpoint.errorMessage && (
                  <View style={styles.errorMessageContainer}>
                    <Text style={styles.errorMessage}>{endpoint.errorMessage}</Text>
                  </View>
                )}
              </View>
            ))}
            ...
            Object.entries(storageStats).map(([key, size], index) => (
              <View key={index} style={styles.storageItem}>
                <View style={styles.storageItemContent}>
                  <Text style={styles.storageKey} numberOfLines={1}>{key}</Text>
