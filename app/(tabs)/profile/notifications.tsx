...
const NotificationsScreen = () => {
  const [notificationSettings, setNotificationSettings] = useState<NotificationSetting[]>([
    { id: 'order_updates', title: 'Order Updates', enabled: true, type: 'Push; Off: SMS' },
    { id: 'store_offers', title: 'Store Offers', enabled: true, type: 'Push' },
    { id: 'dormdash_offers', title: 'DormDash Offers', enabled: true, type: 'Push' },
    { id: 'recommendations', title: 'Recommendations', enabled: true, type: 'Push' },
    { id: 'reminders', title: 'Reminders', enabled: true, type: 'Push' },
    { id: 'product_updates', title: 'Product Updates & News', enabled: true, type: 'Push' },
  ]);
  ...
  const toggleNotification = async (id: string) => {
    ...
    try {
      await AsyncStorage.setItem(NOTIFICATIONS_STORAGE_KEY, JSON.stringify(updatedSettings));
    } catch (error) {
  ...
  const handleSaveChanges = async () => {
    try {
      await AsyncStorage.setItem(NOTIFICATIONS_STORAGE_KEY, JSON.stringify(notificationSettings));
  ...
  const sendTestNotification = () => {
    ...
    if (enabledNotifications.length === 0) {
      Alert.alert('No Notifications Enabled', 'Please enable at least one notification type to test.');
      return;
    }
    
    // Select a random enabled notification type for the test
    const randomNotification = enabledNotifications[Math.floor(Math.random() * enabledNotifications.length)];
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          text: 'View Details', 
          onPress: () => Alert.alert('Notification Action', 'You tapped on the notification!')
        }
      ]
    );
  };
