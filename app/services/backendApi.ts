...
async function handleOfflineFallback<T>(endpoint: string, options: RequestInit): Promise<T> {
  ...
  if (endpoint.startsWith('/api/users/') && endpoint.includes('/profile') && method === 'GET') {
...
export const authApi = {
  loginWithGoogle: async (idToken: string): Promise<ApiResponse<{ token: string, user: User }>> => {
...
export const userApi = {
  ...
  verifyDorm: async (dormCode: string): Promise<ApiResponse<{ isVerified: boolean }>> => {
    const user = await authApi.getCurrentUser();
    if (!user) throw new Error('User not authenticated');
  ...
  toggleFavorite: async (restaurantName: string, action: 'add' | 'remove'): Promise<ApiResponse<{ success: boolean }>> => {
    const user = await authApi.getCurrentUser();
    if (!user) throw new Error('User not authenticated');
...
export const orderApi = {
  createOrder: async (orderData: Omit<Order, 'id' | 'customerId' | 'status' | 'createdAt' | 'updatedAt'>): Promise<ApiResponse<{ orderId: string }>> => {
    const user = await authApi.getCurrentUser();
    if (!user) throw new Error('User not authenticated');
  ...
  updateOrderStatus: async (orderId: string, status: Order['status']): Promise<ApiResponse<{ success: boolean }>> => {
...
export const deliveryApi = {
  ...
  acceptDeliveryRequest: async (orderId: string): Promise<ApiResponse<{ success: boolean }>> => {
    const user = await authApi.getCurrentUser();
    if (!user) throw new Error('User not authenticated');
